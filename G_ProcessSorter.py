...
class ProcessSelector:
    ...
    def __init__(self, root, config):
        ...
        self.supplier_list = sorted(self.config.get("supplier_list", [])) # ソートして読み込む
    ...
    def _build_ui(self):
        ...
        # 1. 工事番号
        ttk.Label(input_frame, text="工事番号:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ...
        # 2. 納品業者 (Comboboxに変更)
        ttk.Label(input_frame, text="納品業者:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ...
        # 3. 追加工程 (Comboboxに変更)
        ttk.Label(input_frame, text="追加工程:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        ...
        # 4. スキャン開始ボタン
        start_button = ttk.Button(main_frame, text="スキャン開始", command=self._on_start_scan, style="Accent.TButton")
    ...
    def _on_start_scan(self):
        ...
        if not process_name:
            messagebox.showwarning("入力エラー", "追加工程を選択してください。", parent=self.root)
            return
...
def main():
    ...
    lock_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "G_ProcessSorter.py.lock")
    try:
    ...
    ttk.Label(splash_root, text="アプリケーションを起動しています...", font=("", 10)).pack(pady=20)
    progress = ttk.Progressbar(splash_root, orient="horizontal", length=250, mode="indeterminate")
    ...
    try:
        ...
        if 'splash_root' in locals() and splash_root.winfo_exists():
            splash_root.destroy()
        messagebox.showerror("実行エラー", f"工程スキャナーの実行中にエラーが発生しました:\n{e}")
    finally:
