...
import os
import sys
import argparse  # 引数解析のためにインポート
from G_config import Config
from G_ScanBCD_FixCSV import CSVHandler  # 共通のCSV読み込みクラスを利用
...
class DrawingNumberViewer:
    def __init__(self, root, config):
        ...
        self.default_source_csv_filename = self.config.get(
            "default_source_csv_filename", ""
        )
        self.order_no_col = self.config.get("source_csv_order_no_column", "発注伝票No.")
        self.drawing_no_col = self.config.get(
            "source_csv_drawing_no_column", "図面番号"
        )
        self.parts_no_col_name = self.config.get(
            "source_csv_parts_no_column", "部品№"
        )  # configから部品№カラム名取得
        self.no_barcode_type_str = self.config.get(
            "no_barcode_type", "NO_BARCODE"
        )  # バーコードなし部品のタイプ
        self.manual_drawing_barcode_type_str = self.config.get(
            "manual_entry_drawing_barcode_type", "MANUAL_DRAWING"
        )  # 図番手動登録タイプ

        # 図番からのキー抽出設定 (config.json から読み込み)
        # ユーザー設定は1始まり、内部処理用に0始まりに変換
        self.key_extract_start_0based = (
            self.config.get("drawing_key_extraction_start_user", 8) - 1
        )
        ...
        elif self.default_source_csv_filename and os.path.isdir(
            self.source_data_dir
        ):  # source_data_dirが存在する場合のみ結合
            self.source_csv_path.set(
                os.path.join(self.source_data_dir, self.default_source_csv_filename)
            )
        ...
        # 工事番号入力
        ttk.Label(main_frame, text="工事番号:").grid(
        ...
        # --- 起動時の工事番号を決定 ---
        # 1. コマンドライン引数 > 2. 前回値 > 3. デフォルト値 の順で優先
        parser = argparse.ArgumentParser()
        parser.add_argument("--cn", help="Initial construction number")
        args, _ = parser.parse_known_args()
        last_construction_no = (
            args.cn
            if args.cn
        ...
        # 発注伝票CSV選択
        ttk.Label(main_frame, text="発注伝票CSV:").grid(
            row=1, column=0, padx=5, pady=5, sticky=tk.W
        )
        self.source_csv_entry = ttk.Entry(
            main_frame, textvariable=self.source_csv_path, width=40
        )
        self.source_csv_entry.grid(row=1, column=1, padx=5, pady=5, sticky=tk.EW)
        self.browse_button = ttk.Button(
            main_frame, text="参照...", command=self.browse_source_csv
        )
        ...
        ttk.Label(
            filter_frame,
            text="部品№ (カンマ区切り可 例: 1,10 → #100,#1000 / 空,0: 全表示):",
        ).grid(row=0, column=0, padx=5, pady=2, sticky=tk.W)
        self.filter_start_entry = ttk.Entry(filter_frame, width=20)  # 幅を調整
        self.filter_start_entry.grid(
            row=0, column=1, columnspan=2, padx=(0, 5), pady=2, sticky=tk.EW
        )  # columnspan調整
        last_filter_start = self.config.get(
            "last_filter_start_value", str(self.default_filter_start_val)
        )
        self.filter_start_entry.insert(0, last_filter_start)

        # 保管場所フィルター (Listboxに変更、複数選択対応)
        ttk.Label(filter_frame, text="保管場所 (複数選択可):").grid(
            row=1, column=0, padx=5, pady=2, sticky=tk.W
        )
        self.location_filter_listbox = tk.Listbox(
            filter_frame, selectmode=tk.EXTENDED, exportselection=False, height=4
        )
        self.location_filter_listbox_scrollbar = ttk.Scrollbar(
            filter_frame, orient=tk.VERTICAL, command=self.location_filter_listbox.yview
        )
        self.location_filter_listbox.configure(
            yscrollcommand=self.location_filter_listbox_scrollbar.set
        )
        self.location_filter_listbox.grid(
            row=1, column=1, columnspan=2, padx=(0, 0), pady=2, sticky=tk.EW
        )
        self.location_filter_listbox_scrollbar.grid(
            row=1, column=3, padx=(0, 5), pady=2, sticky=(tk.N, tk.S)
        )
        self.location_filter_listbox.bind(
            "<<ListboxSelect>>", lambda e: self._apply_filters_and_display()
        )
        ...
        self.match_button = ttk.Button(
            main_frame, text="照合実行", command=self.perform_matching
        )
        ...
        self.tree.heading(
            "order_no", text=self.order_no_col
        )  # 設定ファイルからカラム名を取得
        self.tree.heading("parts_no", text=self.parts_no_col_name)  # 部品№のヘッダー
        self.tree.heading(
            "drawing_no", text=self.drawing_no_col
        )  # 設定ファイルからカラム名を取得
        ...
        self.data_count_label.grid(
            row=5, column=0, columnspan=3, padx=5, pady=5, sticky=tk.E
        )

        # ウィンドウリサイズ設定
        main_frame.columnconfigure(
            1, weight=1
        )  # Entryウィジェットがリサイズされるように
    ...
    def on_closing(self):
        ...
        selected_locations = [
            self.location_filter_listbox.get(i) for i in selected_location_indices
        ]
        ...
        self.config.set(
            ...
            if current_filter_start
            else str(self.default_filter_start_val),
        )
    ...
    def _setup_keyboard_navigation(self):
        ...
        self.filter_start_entry.bind(
            "<FocusIn>", self._select_all_on_focus
        )  # Listboxは対象外

        # Enterキーでのフォーカス移動とアクション
        self.construction_no_entry.bind("<Return>", self.on_construction_no_changed)
        self.source_csv_entry.bind(
            "<Return>", lambda event: self.filter_start_entry.focus_set()
        )
        self.filter_start_entry.bind(
            "<Return>", lambda event: self._apply_filters_and_display()
        )
        self.browse_button.bind(
            "<Return>", lambda event: self.browse_button.invoke()
        )  # Enterでボタン実行
    ...
    def load_scanned_data(self, construction_no):
        ...
        if not os.path.exists(scan_data_filename):
            messagebox.showwarning(
                "警告", f"スキャンデータファイルが見つかりません:\n{scan_data_filename}"
            )
            return {}

        handler = CSVHandler(scan_data_filename, self.config)
        scanned_data_list = handler.load_csv()  # 辞書のリストとして読み込み
        # バーコードをキーにした辞書に変換
        return {
            row.get("barcode_info"): row
            for row in scanned_data_list
            if row.get("barcode_info")
        }
    ...
    def load_source_data(self, filepath):
        ...
        if not filepath or not os.path.exists(filepath):
            messagebox.showwarning(
                "警告",
                f"発注伝票CSVファイルが指定されていないか、見つかりません:\n{filepath}",
            )
            return None
        try:
            with open(
                filepath, mode="r", newline="", encoding="utf-8-sig"
            ) as file:  # utf-8-sigでBOM対応
                reader = csv.DictReader(file)
                # 部品№カラムも必須チェックに含める（ただし、データ自体はなくても良い）
                required_cols = [
                    self.order_no_col,
                    self.drawing_no_col,
                    self.parts_no_col_name,
                ]
                missing_cols = [
            ...
            messagebox.showerror(
                "エラー", f"発注伝票CSVファイルの読み込み中にエラーが発生しました:\n{e}"
            )
            return None
    ...
    def update_location_filter_options(self):
        ...
        unique_locations = sorted(
            list(
                set(
                    info["location"]
                    for info in scanned_data_map.values()
        ...
        if last_location_filters_str:
            ...
            for i, item_in_listbox in enumerate(
                self.location_filter_listbox.get(0, tk.END)
            ):
    ...
    def _get_sort_key(self, drawing_no_str):
        ...
        def sort_key_func(item):
            ...
            if (
                drawing_no_str
                and isinstance(drawing_no_str, str)
                and len(drawing_no_str)
                >= (self.key_extract_start_0based + self.key_extract_length)
            ):
                try:
                    sort_val_str = drawing_no_str[
                        self.key_extract_start_0based : self.key_extract_start_0based
                        + self.key_extract_length
                    ]
    ...
    def _get_status_and_tag(self, source_info, scanned_type):
        ...
        if (
            scanned_type == "MANUAL"
            or scanned_type == self.manual_drawing_barcode_type_str
        ):
    ...
    def _prepare_data_for_display(self, scanned_data_map, source_data_map):
        ...
        for bc, scanned_info in scanned_data_map.items():
            ...
            status, tag = self._get_status_and_tag(
                source_info, scanned_info.get("type", "")
            )
            ...
            sort_key_display = "---"
            if isinstance(drawing_no, str) and len(drawing_no) >= (
                self.key_extract_start_0based + self.key_extract_length
            ):
                sort_key_display = drawing_no[
                    self.key_extract_start_0based : self.key_extract_start_0based
                    + self.key_extract_length
                ]
    ...
    def _filter_results(self, all_results):
        ...
        if filter_input_str and filter_input_str != "0":
            ...
            for val_str in raw_filter_inputs:
                ...
                try:
                    ...
                    messagebox.showwarning(
                        "入力エラー",
                        f"部品№フィルターの '{val_str}' は数字で入力してください。",
                    )
                    return None  # エラーを示す
        ...
        selected_locations = [
            self.location_filter_listbox.get(i) for i in selected_indices
        ]
        all_locations_selected = (
            not selected_indices or "すべての場所" in selected_locations
        )
    ...
    def _apply_filters_and_display(self):
        ...
        def sort_key_func(item):
            ...
            if isinstance(drawing_no_str, str) and len(drawing_no_str) >= (
                self.key_extract_start_0based + self.key_extract_length
            ):
                try:
                    return int(
    ...
    def perform_matching(self):
        ...
        self.prepared_data = self._prepare_data_for_display(
            scanned_data_map, source_data_map
        )
