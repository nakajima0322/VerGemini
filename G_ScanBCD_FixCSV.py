...
class CSVHandler:
        ...
        try:
            with open(self.csv_file, mode="r", newline="", encoding="utf-8") as file:
                ...
                if set(first_row) == set(self.new_header):
                    ...
                    # 古い形式のファイル
                    print(
                        f"情報: 古い形式のファイル '{os.path.basename(self.csv_file)}' を検出しました。保存時に新しい形式に更新します。"
                    )
                    # ヘッダー行のデータを辞書化
                    ...
                    if self.file_type == "location":
                        print(
                            f"情報: ヘッダーのない古い保管場所データファイルを検出しました。保存時に新しい形式に更新します。 ('{os.path.basename(self.csv_file)}')"
                        )
        ...
        for row in data:
            ...
            if len(barcode_info) != self.expected_length:
                if not (self.file_type == "location" and barcode_type == "MANUAL"):
                    print(
                        f"⚠ [不正検出] バーコード長が異なります: '{barcode_info}' (期待値: {self.expected_length})"
                    )
                    is_invalid = True
            ...
            if not (self.file_type == "location" and barcode_type == "MANUAL"):
                if not barcode_info.isdigit():
                    print(
                        f"⚠ [不正検出] バーコードに数字以外の文字が含まれています: '{barcode_info}'"
                    )
                    is_invalid = True
    ...
    def check_data_status(self):
        """
        現在のCSVデータを読み込み、重複件数と不正行数を返す（GUIなし）。
        Returns:
            dict: {"duplicates": int, "invalid": int}
        """
        data = self.load_csv()
        if not data:
        ...
        for row in data:
            ...
            key = row.get(self.primary_key_col, "")
            if key in seen:
                # 重複がある場合、どちらか一方が削除対象となるためカウントする
                duplicates.append(row)
            else:
                seen[key] = row
        
        return {"duplicates": len(duplicates), "invalid": len(invalid_rows)}
    ...
    def find_duplicates_and_invalid_rows(self):
        ...
        for row in data:
            ...
            key = row.get(self.primary_key_col, "")
            if key in seen:
                # 重複が見つかった場合、タイムスタンプを比較して新しい方を残す
                existing_timestamp = seen[key].get(self.timestamp_col, "0")
                current_timestamp = row.get(self.timestamp_col, "0")
                if current_timestamp > existing_timestamp:
                    duplicates.append(seen[key])  # 古い方を削除リストへ
                    seen[key] = row  # 新しい方で上書き
                else:
    ...
    def show_gui(self, unique_data, rows_to_remove):
        ...
        def confirm_removal():
            ...
            rows_to_keep_values = [
                tuple(tree.item(item)["values"]) for item in selected_items
            ]
            rows_to_remove_final = [
                row
                for row in rows_to_remove
                if tuple(str(v) for v in row.values()) not in rows_to_keep_values
            ]
            root.destroy()
        ...
        ttk.Button(button_frame, text="選択した行を削除", command=confirm_removal).pack(
            side=tk.RIGHT, padx=5
        )  # button_frame内ではpackでOK
        ttk.Button(button_frame, text="キャンセル", command=root.destroy).pack(
            side=tk.RIGHT
        )
        root.mainloop()
