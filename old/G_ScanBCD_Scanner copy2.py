...
class BarcodeScanner:
    ...
    def start(self, auto_stop=True):
        ...
        try:
            while True:
                ...
                if barcodes:  # バーコードが検出された場合のみ処理を行う
    ...
    def display(self, frame, barcodes):
        ...
        if display_lines == 1:
            cv2.putText(frame, spec_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), 2, cv2.LINE_AA)
        else:
            spec_text_lines = spec_text.split(" | ")
            for i, line in enumerate(spec_text_lines):
                if i < display_lines:
                    cv2.putText(frame, line, (text_x, text_y + i * 20), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        for info in self.scanned_info:
            ...
            if time.time() - timestamp <= self.display_time:
                ...
                cv2.putText(frame, text, (text_x, text_y + display_lines * 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2, cv2.LINE_AA)
                text_y += 20
