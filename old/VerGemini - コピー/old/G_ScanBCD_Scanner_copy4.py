...
class BarcodeScanner:
    def __init__(self, scan_log, expected_length, location, construction_number, config):
        ...
        self.config = config
        self.last_frame_time = 0 #フレームレート制御用
        self.target_fps = 30 #目標フレームレート
        self.scanned_info = [] # scanned_info 属性を初期化
    ...
    def start(self, auto_stop=True):
        ...
        while True:
            ...
            self.last_frame_time = current_time #フレームレート制御用
...
if __name__ == "__main__":
    ...
    scanner = BarcodeScanner(scan_log="scanned_barcodes_log.csv", expected_length=10, location="Kabuto 1F", construction_number="1234", config=config)
