...
class OverlayDisplay:
    ...
    def get_config_value(self, key, default=None):
        if self.config.get(key) is None:
            if default is not None:
                print(f"警告: 設定ファイルに '{key}' が存在しないため、デフォルト値 '{default}' は使用できません。")
                return None  # default引数がある場合でも、None を返す
            else:
    ...
    def display_overlay(self, frame, barcodes, scanned_info, scan_count, success_count, failure_count, duplicate_count, location, construction_number, remaining_time):
        height, width, _ = frame.shape
        overlay_x = 30
        overlay_y = 30

        text_mapping = self.get_config_value("display_text_mapping")
        display_location = text_mapping.get(location, location)
        spec_text = f"Type: {self.get_config_value('barcode_type', '-')} | Digits: {self.get_config_value('expected_length', '-')} | Location: {display_location} | Construction: {construction_number}"

        font_scale = self.get_config_value("font_scale")
        display_lines = self.get_config_value("display_lines")
    ...
    # 半透明の背景を描画
        ...
        alpha = self.get_config_value("overlay_alpha")
        overlay_color = self.get_config_value("overlay_color")
    ...
    # 残り時間を表示
        ...
        for info in scanned_info:
            ...
            if time.time() - timestamp <= self.get_config_value("display_time"):
            ...
            if time.time() - timestamp > self.get_config_value("display_time"):
                scanned_info[:] = [info for info in scanned_info if time.time() - info['timestamp'] <= self.get_config_value("display_time")]
