...
class LauncherApp:
    def __init__(self, root, config):
        ...
        # --- ★★★ 作業者選択エリア (新規追加) ★★★ ---
        worker_frame = ttk.LabelFrame(main_frame, text="作業者を選択してください", padding="10")
        ...
        self.worker_listbox = tk.Listbox(listbox_container, exportselection=False, height=4)
        self.worker_listbox.grid(row=0, column=0, sticky="nsew")

        scrollbar = ttk.Scrollbar(listbox_container, orient=tk.VERTICAL, command=self.worker_listbox.yview)
        ...
        process_sorter_button = ttk.Button(workflow_frame, text="工程登録スキャン", command=self._run_process_sorter, width=btn_width)
        process_sorter_button.pack(pady=btn_pady)

        scan_button = ttk.Button(workflow_frame, text="保管場所登録スキャン", command=self._run_scan_bcd_main, width=btn_width)
        ...
        worker_manager_button = ttk.Button(admin_frame, text="作業者リスト管理", command=self._open_worker_manager, width=btn_width)
        worker_manager_button.pack(pady=btn_pady)

        csv_fixer_button = ttk.Button(admin_frame, text="データ修正ツール (FixCSV)", command=self._run_csv_fixer, width=btn_width)
        csv_fixer_button.pack(pady=btn_pady)

        data_editor_button = ttk.Button(admin_frame, text="データビューア/エディタ", command=self._run_data_editor, width=btn_width)
    ...
    def _run_tool(self, script_name, args=None):
        ...
        if not selected_indices:
            messagebox.showwarning("作業者未選択", "作業者を選択してください。", parent=self.root)
            return
    ...
    def _open_worker_manager(self):
        ...
        def delete_worker():
            ...
            if not selected_indices:
                messagebox.showwarning("選択エラー", "削除する作業者を選択してください。", parent=manager_win)
                return

            selected_worker = listbox.get(selected_indices[0])
            if messagebox.askyesno("削除確認", f"作業者 '{selected_worker}' をリストから削除しますか？", parent=manager_win):
        ...
        def add_worker():
            """新しい作業者を追加する"""
            new_worker = simpledialog.askstring("作業者を追加", "新しい作業者名を入力してください:", parent=manager_win)
            if new_worker:
                ...
                if new_worker:
                    ...
                    if new_worker not in current_list:
                        ...
                        messagebox.showwarning("追加エラー", f"作業者 '{new_worker}' は既に存在します。", parent=manager_win)
        ...
        ttk.Button(button_frame, text="選択した作業者を削除", command=delete_worker).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="作業者を追加", command=add_worker).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="閉じる", command=manager_win.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def _run_location_viewer(self): # メソッド名も変更
        self._run_tool("G_DrawingNumberViewer.py")

    def _run_part_info_viewer(self):
        self._run_tool("G_PartInfoViewer.py")
